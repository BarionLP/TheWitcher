...
public class TWItems {
    ...
    public static final RegistryObject<Item> TabLogo = TheWitcher.Registers.Items.register("tab_logo", () -> new Item(new Item.Properties()));
    ...
    private static RegistryObject<Item> regItem(String id){
        return TheWitcher.Registers.Items.register(id, () -> new Item(new Item.Properties().tab(TWUtil.TheWitcherTab)));
    }
    private static RegistryObject<TWUtil.Swords.Silver> regSilverSword(String id, int dmgBonus, Rarity rarity){
        return TheWitcher.Registers.Items.register(id, ()-> new TWUtil.Swords.Silver(TWUtil.Tiers.Silver, dmgBonus, 2, new Item.Properties().tab(TWUtil.TheWitcherTab).defaultDurability(1200).rarity(rarity)));
    }
    private static RegistryObject<SwordItem> regSword(String id, Tier tier, int dmg, float speed, int durability){
        return TheWitcher.Registers.Items.register(id, ()-> new SwordItem(tier, dmg, speed, new Item.Properties().tab(TWUtil.TheWitcherTab).defaultDurability(tier.getUses() + durability)));
    }
    private static RegistryObject<SwordItem> regSword(String id, Tier tier, int dmg, float speed, int durability, Rarity rarity){
        return TheWitcher.Registers.Items.register(id, ()-> new SwordItem(tier, dmg, speed, new Item.Properties().tab(TWUtil.TheWitcherTab).defaultDurability(tier.getUses() + durability).rarity(rarity)));
    }
